...
-- 2020-11-27,SusanLee,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_SusanLee')
	 Begin 
	  Alter Database [Assignment07DB_SusanLee] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_SusanLee;
	 End
	Create Database Assignment07DB_SusanLee;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_SusanLee;
...
AS
	...
	FROM
		vCategories AS C INNER JOIN
		vProducts AS P ON
		C.CategoryID = P.CategoryID INNER JOIN
		vInventories AS I ON
		P.ProductID	 = I.ProductID
	GROUP BY
		CategoryName
...
AS
	SELECT TOP 1000000
		...
		,(DateName(mm,InventoryDate) + ', '+ DateName(yy,InventoryDate)) AS InventoryDate
		, SUM([Count]) AS InventoryCount
		,IIF(Year(InventoryDate) = '1996', 0, ISNULL(LAG(SUM([Count])) OVER (ORDER BY ProductName, MONTH(InventoryDate)),0)) AS PreviousMonthCount
	FROM
		vProducts AS P INNER JOIN
		vInventories AS I ON
		P.ProductID = I.ProductID
...
ALTER FUNCTION fProductInventoriesWithPreviousMonthCountsWithKPIs
(@KPI  INT)
RETURNS TABLE
	AS
		RETURN
